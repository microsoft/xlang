...
:main
    ...
    if defined RUN_CMAKE (
        cmake "%SRC_PATH%" "-B%BUILD_PATH%" -G Ninja -DCMAKE_BUILD_TYPE="%BUILD_TYPE%" -DXLANG_BUILD_VERSION="%BUILD_VERSION%" -DCMAKE_INSTALL_PREFIX:PATH="%BUILD_PATH%\Install" -DCMAKE_TOOLCHAIN_FILE="%SRC_PATH%\vcpkg\scripts\buildsystems\vcpkg.cmake"  
    )
