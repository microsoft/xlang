...
:main
    ...
    if defined RUN_CMAKE (
        cmake "%SRC_PATH%" "-B%BUILD_PATH%" %OPT_VERBOSE_CMAKE% -GNinja -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_C_COMPILER=cl -DCMAKE_CXX_COMPILER=cl -DXLANG_BUILD_VERSION=%BUILD_VERSION% "-DCMAKE_INSTALL_PREFIX=%BUILD_PATH%/Install" "-DCMAKE_TOOLCHAIN_FILE=%SRC_PATH%/vcpkg/scripts/buildsystems/vcpkg.cmake"
    )
