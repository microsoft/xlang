...
#include "base.h"
#include "meta_reader/view.h"
#include "meta_reader/enum.h"
#include "meta_reader/enum_traits.h"
#include "meta_reader/table.h"
#include "meta_reader/index.h"

namespace xlang::meta::signature
{
    using namespace meta::reader;
    using meta::reader::begin;
    using meta::reader::end;
    using meta::reader::size;
    ...
    struct signature_blob
    {
        ...
        void add_signature(CustomModSig const& sig)
        {
            add_compressed_enum(sig.CustomMod());
            add_type_ref(sig.Type());
        }

        void add_signature(GenericTypeInstSig const& sig)
        {
            add_compressed_enum(ElementType::GenericInst);
        ...
        void add_signature(TypeSig const& sig)
        {
            if (sig.is_szarray())
            {
                add_compressed_enum(ElementType::SZArray);
            }
            call(sig.Type(),
                [&](ElementType arg)
                {
                ...
                [&](coded_index<TypeDefOrRef> const& arg)
                {
                    ...
                    [&](GenericTypeIndex const& arg)
                {
                    add_compressed_enum(ElementType::Var);
                    ...
                    [&](GenericTypeInstSig const& arg)
                {
                    ...
                    [&](GenericMethodTypeIndex const& arg)
                {
                    add_compressed_enum(ElementType::MVar);
                    add_compressed_unsigned(arg.index);
                }
                );
        }

        void add_signature(FieldSig const& sig)
        {
            add_compressed_enum(sig.get_CallingConvention());
            add_signature(sig.Type());
        }

        void add_signature(ParamSig const& sig)
        {
            for (auto const& cmod : sig.CustomMod())
            {
                ...
                if (sig.ByRef())
                {
                    add_compressed_enum(ElementType::ByRef);
                }
                add_signature(sig.Type());
            }
        }

        void add_signature(RetTypeSig const& sig)
        {
            for (auto const& cmod : sig.CustomMod())
            {
                add_signature(cmod);
            ...
            {
                add_compressed_enum(ElementType::Void);
            }
        }

        void add_signature(MethodDefSig const& sig)
        {
            add_compressed_enum(sig.CallConvention());
            if (enum_mask(sig.CallConvention(), CallingConvention::Generic) == CallingConvention::Generic)
            {
        ...
        void add_signature(PropertySig const& sig)
        {
            add_compressed_enum(sig.CallConvention());
        ...
        void add_signature(TypeSpecSig const& sig)
        {
