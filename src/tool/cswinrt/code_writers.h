...
namespace cswinrt
{
    ...
    void write_iid_field(writer& w, TypeDef const& type)
    {
        ...
        w.write_printf("public static readonly System.Guid IID = new System.Guid(0x%08Xu,0x%04X,0x%04X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X);\n\n",
...
internal %(System.IntPtr instance)
{
    ...
    {
        call(semantics,
            [&](object_type) { w.write("System.IntPtr"); },
            [&](guid_type) { w.write("System.Guid"); },
            [&](type_definition const&) { w.write("System.IntPtr"); },
            [&](generic_type_index const& var) { w.write(w.generic_param_stack.back()[var.index]); },
        ...
        w.write("internal static class _@Interop\n{\n", name);
        {
            ...
            for (auto&& method : type.MethodList())
            {
                    ...
                    bind<write_abi_parameters>(signature));
                {
                    ...
                    w.write("var __delegate = System.Runtime.InteropServices.Marshal.GetDelegateForFunctionPointer<%>(new System.IntPtr(__slot));\n", 
                        bind<write_interop_method_name>(method, "abi", offset));
