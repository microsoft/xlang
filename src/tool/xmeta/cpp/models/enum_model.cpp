...
namespace xlang::xmeta
{
    ...
    std::errc enum_member::increment(enum_type type)
    {
        switch (type)
        {
        case enum_type::Int8:
    ...
    std::errc enum_member::resolve_decimal_val(enum_type type)
    {
        assert(!m_value.is_resolved());
        return resolve_numeric_val(type, 10);
    }

    std::errc enum_member::resolve_hexadecimal_val(enum_type type)
    {
        assert(!m_value.is_resolved());
        return resolve_numeric_val(type, 16);
    }

    std::errc enum_member::resolve_numeric_val(enum_type type, int base) noexcept
    {
        switch (type)
        {
        case enum_type::Int8:
