...
namespace xlang
{
    ...
    {
        auto format = R"(// WARNING: Please don't edit this file. It was generated by javax v%
)";
...
static_assert(winrt::check_version(CPPWINRT_VERSION, "%"), "Mismatched javax headers.");
)";
...
%    };
    ...
    {
        auto format = R"(// Java/xlang projection for % namespace
...
#define JNI_EXPORT_NAMESPACE %
    ...
    generic_info[] =
    {
        {"Iterable"sv, "iterator_type"sv, "Iterator"sv, "java.lang.Iterable"},
    };

    static auto get_generic_base_info(writer& w, TypeDef const& type)
    {
        std::map<generic_info_type, interface_info> generic_bases;

        for (auto&&[interface_name, info] : get_interfaces(w, type))
        {
            // todo: filter out non-generic interfaces
            // note: some interfaces are lacking exclusivto attribute (e.g., IWwwFormUrlDecoderEntry)
            ...
            if (starts_with(interface_name, collections_namespace))
            {
                ...
                static constexpr std::string_view iterable("IIterable<"sv);
                //static constexpr std::string_view vector_view("IVectorView<"sv);
                //static constexpr std::string_view map_view("IMapView<"sv);
                //static constexpr std::string_view vector("IVector<"sv);
                //static constexpr std::string_view map("IMap<"sv);
                if (starts_with(relative_type, iterable))
                {
                    generic_bases[generic_info_type::Iterable] = std::move(info);
                }
            }
        };
    ...
    static void write_jni_generic_base(writer& w, std::pair<generic_info_type, interface_info> const& generic_base, std::string_view const& derived_type)
    {
        w.write(", %<%>", generic_info[to_underlying(generic_base.first)].base_class, derived_type);
    }
...
JNI_EXPORT(%);
    ...
    static void write_jni_generic_trait(writer& w, std::pair<generic_info_type, interface_info> const& generic_base)
    {
        // todo: find classes for exclusive-to generic interface params
        auto cpp_params = w.write_temp("%", bind_list(",", generic_base.second.generic_param_stack.back()));
        ...
        auto name = w.write_temp("%%", generic_info[to_underlying(generic_base.first)].projected, params);

        if (generic_base.first == generic_info_type::Iterable)
        {
            std::string element;
        ...
        w.write(format,
            generic_info[to_underlying(generic_base.first)].base_trait,
            java_type_name{ name, w.type_namespace });
    }

    static void write_jni_generic_register(writer& w, std::pair<generic_info_type, interface_info> const& generic_base)
    {
        auto format = R"(        %::Register(env, cls);
)";
        w.write(format, generic_info[to_underlying(generic_base.first)].base_class);
    }

    static void write_jni_constructors(writer& /*w*/, TypeDef const& type)
    {
        // TODO: stable numeric suffix scheme for ctors
        for (auto&& ctor: get_constructors(type))
        {
            ctor;
        }
    }
        ...
        auto format = R"(    static auto abi_%(jni_env&, jobject, jlong abi%)
    {
        ...
        w.write(format,
            method.Name(),
            bind_list<write_param_as<jni_type>>(", ", signature.params(), ", % %"),
            method.Name(),
            bind_list<write_method_arg>(", ", signature.params())
        );
    }

    static void write_jni_methods(writer& w, TypeDef const& type)
    {
        for (auto&&[interface_name, info] : get_interfaces(w, type))
        {
            // todo: filter out non-generic interfaces
            ...
            w.write("%", bind_each<write_jni_method>(info.type.MethodList()));
        }
    }

    static void write_jni_method_descriptor(writer& w, method_signature const& signature)
    {
        w.write("(%)%",
    ...
    static void write_jni_registration(writer& w, MethodDef const& method)
    {
        method_signature signature{ method };
        ...
        w.write(format,
            method.Name(),
            bind<write_jni_method_descriptor>(signature)
        );
    }

    static void write_jni_registrations(writer& w, TypeDef const& type)
    {
        for (auto&&[interface_name, info] : get_interfaces(w, type))
        {
            // todo: filter out non-generic interfaces
            ...
            w.write("%", bind_each<write_jni_registration>(info.type.MethodList()));
        }
    }
...
JNI_EXPORT(%);
        ...
        auto generic_bases = get_generic_base_info(w, type);

        w.write(format, 
            type.TypeName(),
            type.TypeNamespace(),
            type.TypeName(),
            bind_each<write_jni_generic_base>(generic_bases, type.TypeName()),
            java_type_name{ type },
            bind_each<write_jni_generic_trait>(generic_bases),
            bind<write_jni_constructors>(type),
            bind<write_jni_methods>(type),
            bind_each<write_jni_generic_register>(generic_bases),
            bind<write_jni_registrations>(type),
            type.TypeName()
            );
    }
...
%%}
    ...
    static void write_java_implements(writer& w, std::map<std::string, interface_info> interfaces, TypeDef const& type)
    {
        ...
        for (auto&&[interface_name, info] : interfaces)
        {
            ...
            if (starts_with(interface_name, collections_namespace))
            {
                ...
                if (starts_with(relative_type, iterable))
                {
                    implements.push_back("java.lang.Iterable<WwwFormUrlDecoderEntry>");
                    substituted = true;
                }
            }
            if(!substituted)
            {
                implements.push_back(interface_name);
            }
        }
        ...
        w.write(R"(implements %)", bind_list(", ", implements));
    }
    ...
    static void write_java_constructor(writer& w, MethodDef const& method, TypeDef const& type)
    {
        ...
        w.write(format,
            ...
            bind_each<write_param_type_as<java_descriptor_brief>>(signature.params()),
            bind_list<write_method_arg>(", ", signature.params()));
    }
...
%
    ...
    static {
        ...
        w.write(format,
            ...
            type.TypeName(),
            bind<write_java_implements>(interfaces, type),
            type.TypeName(),
            type.TypeName(),
            type.TypeName(),
