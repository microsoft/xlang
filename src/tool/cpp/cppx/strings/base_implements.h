...
namespace xlang::impl
{
    ...
    template <typename T>
    struct is_interface : std::disjunction<std::is_base_of<IObject, T>, is_fast_interface<T>, std::conjunction<std::is_base_of<::IUnknown, T>, std::negation<is_implements<T>>>> {};

#else

    template <typename T>
    struct is_interface : std::disjunction<std::is_base_of<IObject, T>, is_fast_interface<T>> {};
    ...
    struct is_cloaked : std::disjunction<
        std::is_same<IObject, I>,
        std::negation<std::is_base_of<IObject, I>>
    > {};
...
#pragma warning(suppress: 4307)
    ...
    struct INonDelegatingInspectable : IUnknown
    {
    ...
    struct weak_ref : IWeakReference, weak_source_producer<Agile>
    {
        ...
        int32_t WINRT_CALL QueryInterface(guid const& id, void** object) noexcept override
        {
            if (is_guid_of<IWeakReference>(id) || is_guid_of<IUnknown>(id))
            {
    ...
    protected:
        ...
        {
            if (is_guid_of<IObject>(id) || is_guid_of<IUnknown>(id))
            {
    ...
    private:
        ...
        using is_inspectable = std::disjunction<std::is_base_of<IObject, I>...>;
...
WINRT_EXPORT namespace xlang
{
    ...
    auto make(Args&&... args)
    {
        ...
        if constexpr (std::is_same_v<I, IActivationFactory>)
        {
