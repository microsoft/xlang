...
WINRT_EXPORT namespace xlang
{
    ...
    private:
        ...
        struct iterator final : Version::iterator_type, implements<iterator, IIterator<T>>
        {
            void abi_enter()
            {
                m_owner->abi_enter();
    ...
    {
        IVectorView<T> GetView() const noexcept
        {
    ...
    struct observable_vector_base : vector_base<D, T>
    {
        ...
        void SetAt(uint32_t const index, T const& value)
        {
            ...
            call_changed(CollectionChange::ItemChanged, index);
        }

        void InsertAt(uint32_t const index, T const& value)
        {
            vector_base<D, T>::InsertAt(index, value);
            call_changed(CollectionChange::ItemInserted, index);
        }

        void RemoveAt(uint32_t const index)
        {
            vector_base<D, T>::RemoveAt(index);
            call_changed(CollectionChange::ItemRemoved, index);
        }

        void Append(T const& value)
        {
            vector_base<D, T>::Append(value);
            call_changed(CollectionChange::ItemInserted, this->Size() - 1);
        }

        void RemoveAtEnd()
        {
            vector_base<D, T>::RemoveAtEnd();
            call_changed(CollectionChange::ItemRemoved, this->Size());
        }

        void Clear()
        {
            vector_base<D, T>::Clear();
            call_changed(CollectionChange::Reset, 0);
        }
    ...
    private:
        ...
        void call_changed(CollectionChange const change, uint32_t const index)
        {
            m_changed(static_cast<D const&>(*this), make<args>(change, index));
        }

        struct args final : implements<args, IVectorChangedEventArgs>
        {
            args(CollectionChange const change, uint32_t const index) noexcept :
                m_change(change),
                m_index(index)
            {
            }

            CollectionChange CollectionChange() const noexcept
            {
                return m_change;
            }
        ...
        private:
            ...
            xlang::CollectionChange const m_change;
            uint32_t const m_index;
        };
    };

    template <typename D, typename K, typename V, typename Version = impl::no_collection_version>
    struct map_view_base : iterable_base<D, IKeyValuePair<K, V>, Version>
    {
        ...
        void Split(IMapView<K, V>& first, IMapView<K, V>& second) const noexcept
        {
            first = nullptr;
            second = nullptr;
        }
    ...
    struct observable_map_base : map_base<D, K, V>
    {
        ...
        bool Insert(K const& key, V const& value)
        {
            ...
            call_changed(CollectionChange::ItemInserted, key);
            return result;
        }
        ...
        void Remove(K const& key)
        {
            ...
            call_changed(CollectionChange::ItemRemoved, key);
        }

        void Clear() noexcept
        {
            map_base<D, K, V>::Clear();
            call_changed(CollectionChange::Reset, impl::empty_value<K>());
        }

    private:

        event<MapChangedEventHandler<K, V>> m_changed;

        void call_changed(CollectionChange const change, K const& key)
        {
            m_changed(static_cast<D const&>(*this), make<args>(change, key));
        }

        struct args final : implements<args, IMapChangedEventArgs<K>>
        {
            args(CollectionChange const change, K const& key) noexcept :
                m_change(change),
                m_key(key)
            {
            }
